...
|---|---|
| What an agent is allowed to know or type | The README command table and the activation block. Do not leak store paths into the agent interface. CLI output stays silent on git. The activation block tells agents to publish the files the script wrote. |
