...
Tests are pytest as configured in `pytest.ini` (`testpaths = tests`). Iterate with `tox -e py311` (or a single test/file under that env). The full local matrix is `tox run-parallel` (`tox.ini` env_list py311–py314, `package = skip`, pytest `--basetemp` per env under `TMPDIR` so tmp_path is outside the git worktree). Sequential `tox` is the same envs one after another; two concurrent tox processes on the same env in one checkout stomp. Missing interpreters are skipped (`skip_missing_interpreters = true`). If tox is not on `PATH`, prefix with `uvx --with tox`. Do not use this machine's bare `python3` (3.10). Coverage is opt-in via `tox -e coverage` (`pytest-cov --cov=summem` → `coverage/lcov.info`); the default matrix stays coverage-free. CI uploads that lcov report. Tests load repo-root `summem` once per session via a pytest fixture (`SourceFileLoader`; the path has no `.py` suffix). There is no test-result cache: this suite is heavy on `tmp_path`, git worktrees, and a no-suffix script, and coverage-based selection is not proven safe here. Executable behavior is TDD-governed by `.cursor/rules/shared/always-tdd.mdc`. The process-level git, nap-rejection, and scope tests named in `productContext.md` Success Criteria are product tests, not a change-detector on a document.
