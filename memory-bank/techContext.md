...
License: GNU AGPL v3, in `LICENSE`. Additional permission for running (including by an AI agent) and 0BSD terms for the agent prompt template are in the `summem` header (authoritative). `surgery.py` echoes the running permission only.
