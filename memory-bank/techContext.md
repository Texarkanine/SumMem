...
The first backend specified in `VISION.md` is a Python 3 script that writes ordinary files in git. Agents talk to a stable CLI (`wake`, `note`, `nap`, `recall`, `zoom`, `start`). The on-disk format may later change, including to sqlite; the CLI table in `VISION.md` must not.
...
No runtime pin or install path exists until a project manifest or version file is added. When one appears, it is the source of truth for how to run the script.
