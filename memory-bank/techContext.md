...
Activation is the SumMem block at the top of committed `AGENTS.md`. Presence of the driver is not. `init` prints that block. Tests load repo-root `summem`. This repository commits `.summem/notes/` (and naps when written). `.gitignore` does not ignore them.
