...
## What Was Done
- Program stays AGPL. Prompt template is 0BSD (full terms in the `summem` header). No REUSE. `LICENSE` stays verbatim AGPL. Script is the authoritative source; `surgery.py` stays stock AGPL (operator: no echo); README points at the header.
