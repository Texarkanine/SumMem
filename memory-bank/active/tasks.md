...
- `migrate.py` on a complete five-part-64 **grain-8** pair with nap `id`s at two nested depths (outer grain-4 children, inner grain-2 grandchildren): every child **and grandchild** nap `id` becomes 16 hex. A one-level walk of `tree.kids` must fail this test. Dest variant hashes the fully rewritten canonical tree bytes.
...
- `tests/test_migrate.py`: plant 64-hex fixtures with `hashlib` (do not round-trip `write_nap` then strip a now-16-hex field). Cover 4-part and 5-part-64, including a nested nap `id` at one level (grain-4) **and** at two depths (grain-8).
...
1. Stub tests: rewrite `tests/test_migrate.py` so helpers plant **64-hex** stems with `hashlib` (full digest), not `write_nap` then strip variant. Keep: complete 4-part rename, second-run no-op, incomplete skip, `--path`, default root+catalog. Add: complete 5-part-64 grain-2 rewrite; complete 5-part-64 grain-4 nested `id` 64→16 and new `variant_tag`; complete 5-part-64 **grain-8** with 64-hex `id`s at two nested depths — assert every nap `id` in `loads_tree(dest.tree)` (children and grandchildren) has `len==16`, and dest stem equals `nap_stem(seq, full[:16], 8, rewritten_tree_bytes, caption_bytes)`; 5-part-16 untouched; dest-exists skip; unreadable `.tree` on an old stem prints incomplete and exits 1. Oracle dest is `m.nap_stem(seq, full[:16], grain, rewritten_tree_bytes, caption_bytes)` and rewritten tree is `dumps_tree` after truncating `NapChild.id` where `len(id)==64` at **every** depth.
...
4. Write code and run green: `_migrate_store` for each old complete pair: `loads_tree`, `_shorten_tree`, `rewritten_tree_bytes = dumps_tree(...)`, `dest = nap_stem(..., leafset[:16], ..., rewritten_tree_bytes, caption_bytes)`. If dest exists, skip silently. Else **persist the rewritten buffers**, then remove the old names: `m._write_pair(naps, dest, rewritten_tree_bytes, caption_bytes)` (temp-replace `{dest}.tree` then `{dest}.summ` with the hashed bytes), then if `dest != stem` unlink the source `.tree` and `.summ`. Do **not** `Path.replace` the source `.tree` onto dest — that installs the old 64-hex bytes under the new stem. Caption content is unchanged (`caption_bytes` is the on-disk `.summ` read). If `loads_tree` raises `_TREE_PARSE_ERRORS`, print `incomplete pair: {stem}`, skip, remember failure. 5-part-16: `_old_stem` is `None`, leave files. Update the module docstring. `--path` / `started_stores` unchanged.
...
- No tests: unit 2 owns migrate behavior; this unit applies that tool to tracked artifacts. Root grain-32 / 16 / 8 trees already nest 64-hex `"id"` fields at multiple depths; dogfood grain-4 is one nested level. This clone’s rewrite therefore exercises recursion, but the grain-8 fixture in unit 2 is what still pins it after these files are 16-hex.
...
- **`Path.replace` of source `.tree` onto dest:** destination stem would hash rewritten bytes while the file still held 64-hex ids. Mitigation: unit 2 step 4 writes via `_write_pair` then unlinks sources.
...
- **`_shorten_tree` walked only the first kid level:** grain-4 tests would pass; grandchild ids would stay 64 hex. Response: unit 2 step 1 plants a grain-8 two-depth fixture that a one-level walk cannot satisfy; step 2 recurses with `m._replace`.
- **`Path.replace` moved old `.tree` bytes under the new stem:** dest name and contents would disagree. Response: unit 2 step 4 uses `_write_pair` then unlink. Already covered by Challenge.
