...
- `how_to_text()` → a section that starts with `== SumMem Usage ==`, names `{AGENT_BIN}`, teaches note/nap-already-stored/do-not-retry, dated-leaf zoom grammar, recall, and catalog pull (`wake --path` plus ignore-if-no-catalog). Omits `git add`, `notes/`, `naps/`, `Run:`, and `must still be true after a fresh clone`.
- `how_to_text()` → is not `usage_text()` (operator `-h` catalog stays `summem`, not `.summem/summem`).
- `prompt_text()` → bootstrap: H1, `{AGENT_BIN}`, skip keyed on a readable Usage block, `note`, writer-only / untracked / invent filenames. Omits `x1 YYYY-MM-DD`, `before any other tool call`, `git add`, `notes/`, `naps/`, and the old “prior **root** SumMem wake” skip sentence.
...
- `WAKE_LINES` / fold request unchanged when Usage is present (Usage is not a view row).
...
3. Write tests and run red: section header; `{AGENT_BIN}` / note / nap-already-stored / do not retry / `x1 YYYY-MM-DD` / zoom target / recall / `wake --path` / ignore `--path` without catalog; forbid `git add`, `notes/`, `naps/`, `Run:`, `must still be true after a fresh clone`; `how_to_text() != usage_text()` and `.summem/summem` in how-to while `usage_text()` keeps `summem` and omits `.summem/summem`.
...
1. Stub tests: retarget `test_prompt_text_invariants` (add Usage-block skip tokens; drop grammar / old skip sentence). Keep `test_prompt_text_notes_are_part_of_the_work`, lockstep tests, `test_init_prints_recipe_and_prompt`.
2. Stub interface: none (function exists).
3. Write tests and run red: bootstrap names Usage as the skip key; omits `x1 YYYY-MM-DD` and “prior **root** SumMem wake”; still has `note`, writer-only, `{AGENT_BIN}`, no `before any other tool call`. Lockstep tests go red when the committed files still have the fat prompt.
...
- Files: `summem` (`main` wake branch), `tests/test_scopes.py`, `tests/test_cli.py`, `tests/test_proof_ingest.py`, `tests/test_proof_scopes.py`
- Creative ref: same (document order; pull omit; catalog-section pins)

1. Stub tests: replace exact stdout strings in `test_empty_root_omits_project_root_header` and `test_root_only_wake_labels_nonempty_document`; add `test_root_wake_starts_with_usage` and `test_pull_wake_omits_usage`; narrow `test_root_wake_catalog_is_labeled_paths_not_commands` so `lines[0]` is Usage and `wake --path` is forbidden in the catalog section only; drop whole-stdout `.summem/summem` forbids in `test_root_wake_catalogs_other_store` and `test_proof_scopes.py` (keep `summem wake --path pkg` out of the catalog section); `test_proof_ingest.py` finds `== Project-root Memories ==` instead of requiring `lines[0]`.
...
1. README Quick Start: insert the bootstrap file; say existing fat prefixes are replaced once; day-to-day notes that root `wake` prints current usage.
