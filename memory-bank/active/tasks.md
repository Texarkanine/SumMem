...
4. Write code and run green: `reaches` and `zoom_reaches` still treat `sentence in` zoom output as the hit (CLI for `zoom_reaches`, `zoom_text` for `reaches`). Child ids come from `_nap_child_ids` / `Tree.kids`, never from stdout. `zoom_reaches` loads the driver via `SourceFileLoader` on `SCRIPT` (do not import `conftest` — `conftest` already imports `gitutil`). Do not enqueue leaves.
