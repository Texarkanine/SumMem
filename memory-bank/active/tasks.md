...
- **Atlas / `systemPatterns.md`**: “64-hex stays on disk” / “Filenames and `.tree` identity stay 64 hex” / Identity “Stored names keep the full id” become 16 hex stored, unique-prefix displayed. Change-surfaces row for `migrate.py` covers both old grammars and nested-id rewrite. #61 path-length “do not shorten the leaf-set id” is retired here (id stays **in** the filename; width is 16).
...
- New test files: none. Extend `tests/test_codec.py`, `tests/test_migrate.py`, `tests/test_nap_variants.py`, and adjust length assertions in `tests/test_wake.py` and `tests/test_caption_conflict.py`. Hardcoded `"c" * 64` / `"b" * 64` **leafset** fixtures in `tests/test_codec.py` become `* 16`. Unknown-id sentinels (`"0" * 64` in zoom/nap CLI) may stay: they are not stored ids.
...
2. Stub interface: replace `_four_part_stem` with a migrate-only `_old_stem(stem) -> (stamp, rand, leafset64, grain) | None` accepting 4-part-64 and 5-part-64 only. Add `_shorten_tree(m, tree) -> Tree` walking `NapChild` ids. No `summem` export for old stems.
...
### 3. Atlas and systemPatterns — prose/policy

- Files: `docs/architecture/index.md`, `memory-bank/systemPatterns.md`
- No tests: prose/policy artifact

1. Identity: stored leaf-set id is 16 hex; wake/recall/zoom unique-prefix that field; do not say “stored names keep the full 64-hex id.”
...
### 4. This clone's stores — prose/policy

- Files: `.summem/naps/*`, `dogfood/.summem/naps/*`
- No tests: on-disk artifacts of this repository, not product behavior. Product migrate behavior is unit 2.

1. After units 1–2 are green, run `migrate.py` from the repository root (default: all started stores).
2. Confirm root and dogfood naps are five-part-16, nested `"id"` values are 16 hex, and `.summem/summem wake` still lists packs.
...
- **Migrate tests kept `_legacy_complete_pair` via current `write_nap`:** they would plant the new grammar and never exercise 64-hex. Response: Challenge 1; unit 2 forbids that helper shape.
- **#61 “do not shorten” left in the atlas so the next reader thinks 64 hex is still required:** unit 3 rewrites those sentences. Already covered.
