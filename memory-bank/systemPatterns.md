...
Session start wakes the true root once, because of the `AGENTS.md` block, not a harness hook. Skip if a prior project-root wake is still readable in the conversation. That print includes Usage, then the catalog: walk the tree, honor git ignore (including `.git/info/exclude`), do not keep a committed index. The pull recipe lives in Usage, not in the bootstrap. Catalog lines are paths. `wake --path` does not reprint Usage, root, or the full catalog. Do not load every started store in the root wake.
