...
Tests load repo-root `summem` (the path has no `.py` suffix). Iterate with `tox -e py311`, or a single test or file under that env. The full local matrix is `tox run-parallel`: pytest on `tests/` for every declared non-EOL CPython from the 3.11 floor (3.11–3.14) that is installed, concurrently. Do not start two tox processes on the same env in one checkout. If tox is not on `PATH`, prefix the same command with `uvx --with tox` (for example `uvx --with tox tox run-parallel`). Pull requests and pushes to `main` run that same matrix as separate CI jobs (`tox -e py311` … `py314`). `tox -e coverage` is the opt-in lcov command (`coverage/lcov.info`); the default matrix does not pass `--cov`. CI runs it and uploads to Codecov only on pushes to `main`. The Codecov badge 404s until the `CODECOV_TOKEN` repository secret exists and CI has uploaded once.
