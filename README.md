...
Agents invoke `.summem/summem`. Bare `summem` is the printed name of this invocation. If `note` asks for a nap, the note is already stored; do that nap before the next action and do not retry the same note. Never edit store files by hand.
